...
sealed interface AdvancedStatisticsUiState {
	data object NoProvider : AdvancedStatisticsUiState
...
class AdvancedStatisticsViewModel(
	...
	init {
		viewModelScope.launch {
			insightsRepository.activeProvider.collectLatest { active ->
				if (active is ActiveProvider.Connected) {
					refreshData()
				} else {
					_state.value = AdvancedStatisticsUiState.NoProvider
				}
			}
		}
	}
	...
	fun refreshData() {
		viewModelScope.launch {
			...
			try {
				val scrobbles = insightsRepository.getScrobblesSince(0)
