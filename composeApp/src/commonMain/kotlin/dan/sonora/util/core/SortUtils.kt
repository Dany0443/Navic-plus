...
fun ImmutableList<DomainSong>.sortedByListType(
	...
	return when (listType) {
		...
		DomainSongListType.SmartMostPlayed -> sortedByDescending { it.playCount }
		...
		DomainSongListType.LocalMusic -> sortedBy { it.title.lowercase() }
		...
		DomainSongListType.OnRepeat -> filter { it.playCount > 1 }.sortedByDescending { it.playCount }
		DomainSongListType.SmartOnRepeat -> filter { it.playCount > 1 }.sortedByDescending { it.playCount }
		DomainSongListType.NeverPlayed -> filter { it.playCount == 0 }
		DomainSongListType.SmartNeverPlayed -> filter { it.playCount == 0 }
...
fun DomainAlbumListType.toSqlQuery(): RoomRawQuery {
	...
	when (this) {
		...
		DomainAlbumListType.AlphabeticalByAlbumArtist ->
			orderBy = "LOWER(artistName) ASC, LOWER(name) ASC"
