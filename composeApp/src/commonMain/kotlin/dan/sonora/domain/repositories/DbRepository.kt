...
class DbRepository(
	...
	suspend fun syncLibrarySongs(
		...
		// Album details are not a complete catalogue: valid tracks can be present only in
		// playlists (for example when their album metadata is unavailable).  Retain those
		// rows so the main library's SELECT * query and playlist/search paths share the
		// same complete song set.
